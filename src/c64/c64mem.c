...
{
    if(addr < 0x1000) {
        return true; /* the lower 4K is always visible */
    }
    if(bank <= 1 )  {/* default and cpu */
        return true;
    }
    if( addr < 0x8000 ) {
        if( mem_config >= 16 && mem_config <= 23) { /* ultimax mode so this is "open bus" */
            return false; 
        }
        return true; /* this is just ram then*/
    }
    if( addr < 0xA000) {
        switch(mem_config) {
            case 3:
