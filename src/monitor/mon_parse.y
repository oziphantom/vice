...
%token CMD_MEM_DISPLAY CMD_BREAK CMD_BBREAK CMD_TRACE CMD_IO CMD_BRMON CMD_COMPARE
...
checkpoint_rules: CMD_BREAK opt_mem_op address_opt_range opt_if_cond_expr end_cmd
                  {
                      if ($2) {
                          temp = mon_breakpoint_add_checkpoint(0, $3[0], $3[1], TRUE, $2, FALSE);
                      } else {
                          temp = mon_breakpoint_add_checkpoint(0, $3[0], $3[1], TRUE, e_exec, FALSE);
                      }
                ...
                | CMD_UNTIL address_opt_range end_cmd
                  {
                      mon_breakpoint_add_checkpoint(0, $2[0], $2[1], TRUE, e_exec, TRUE);
                  }
                | CMD_UNTIL end_cmd
                  { mon_breakpoint_print_checkpoints(); }
                ...
                | CMD_WATCH opt_mem_op address_opt_range opt_if_cond_expr end_cmd
                  {
                      if ($2) {
                          ...
                          temp = mon_breakpoint_add_checkpoint(0, $3[0], $3[1], TRUE, e_load | e_store, FALSE);
                      }
                      mon_breakpoint_set_checkpoint_condition(temp, $4);
                  }
                | CMD_WATCH end_cmd
                ...
                | CMD_TRACE opt_mem_op address_opt_range opt_if_cond_expr end_cmd
                  {
                      if ($2) {
                          temp = mon_breakpoint_add_checkpoint(0, $3[0], $3[1], FALSE, $2, FALSE);
                      } else {
                          temp = mon_breakpoint_add_checkpoint(0, $3[0], $3[1], FALSE, e_exec | e_load | e_store, FALSE);
                      }
                      mon_breakpoint_set_checkpoint_condition(temp, $4);
                  }
                | CMD_TRACE end_cmd
...
cond_operand: register    { $$ = new_cond;
              ...
              |  '@' address {$$=new_cond;
                          $$->operation=e_INV;
                          $$->is_parenthized = FALSE;
                          $$->banknum=0; $$->value = $2; $$->is_reg = FALSE;
                          $$->child1 = NULL; $$->child2 = NULL;  
                      }                  
               ;
